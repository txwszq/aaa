/**
 * @(#)JsonUtil.java 2013-12-20 Copyright 2013 it.kedacom.com, Inc. All rights
 *                   reserved.
 */
...
public class JsonUtil {
			...
			for (int i = 0; i < jsonArray.size(); i++) {
				...
				switch (getJSONType(jsonvalue)) {
					case JSON_TYPE_OBJECT: {
						 ...
						 * Class<?> newClazz = null; if (subObject.has("class"))
						 * { String className = subObject.getString("class"); if
						 * (className != null) { if (map != null &&
						 * map.get(className) != null) { newClazz =
						 * Class.forName(map.get(className)); } else if
						 * (basePackage != null) { newClazz =
	...
	/**
	 * 在JSON->Object的时候 根据JSON的键(key) 生成Field的值(value)
	 * @param field
	 * @param infoObject
	 * @param key
	 * @param basePackage
	...
	private static Object getFieldValue(Field field, JSONObject infoObject, String key, String basePackage, Map<String, String> map) throws Exception {
		...
		} else if (fieldClass.getSimpleName().toString().equalsIgnoreCase("Float")) {
			fieldValue = infoObject.getString(key);
		} else if (fieldClass.getSimpleName().toString().equalsIgnoreCase("Date")) {
	...
	private static void addObjectToBuffer(StringBuffer xmlBuffer, Object obj, String basePackage, boolean createClass) {
		...
		for (int i = -1; i < fields.size(); i++) {
			...
			try {
				...
				String methodName = "is" + ((char) (fieldName.charAt(0) - 32)) + fieldName.substring(1);
				try {
					method = obj.getClass().getMethod(methodName);
				} catch (Exception e1) {
				}
